...
static enum ice_status ice_acl_init_tbl(struct ice_hw *hw)
{
	...
	for (i = 0; i < ICE_AQC_MAX_ACTION_MEMORIES; i++) {
		u8 member_of_tcam = tbl->act_mems[i].member_of_tcam;
		u16 end = ICE_AQC_MAX_ACTION_ENTRIES - 1;
		u16 act_entry_idx;
		u16 start = 0;
		...
		if (member_of_tcam == tbl->first_tcam)
			start = tbl->first_entry;

		if (member_of_tcam == tbl->last_tcam)
			end = tbl->last_entry;
...
ice_acl_create_tbl(struct ice_hw *hw, struct ice_acl_tbl_params *params)
{
	...
	width = roundup(params->width, ICE_AQC_ACL_KEY_WIDTH_BYTES);
			 ...
			 u16 tcam_idx_in_cascade)
{
	...
	for (j = 0; j < ICE_AQC_ACL_KEY_WIDTH_BYTES; j++) {
		u8 val = ICE_AQC_ACL_BYTE_SEL_BASE + idx;
		    ...
		    u16 *scen_id)
{
	...
	/* set the TCAM select and start_cmp and start_stop bits */
	k = first_tcam;
...
enum ice_status ice_acl_destroy_tbl(struct ice_hw *hw)
{
	...
	/* Mark all the created scenario's TCAM to stop the packet lookup */
	list_for_each_entry(pos_scen, &hw->acl_tbl->scens, list_entry) {
	...
	/* call the aq command to destroy the ACL table with these values */
	...
	list_for_each_entry_safe(pos_scen, tmp_scen, &hw->acl_tbl->scens,
				 list_entry) {
		list_del(&pos_scen->list_entry);
		devm_kfree(ice_hw_to_dev(hw), pos_scen);
	}
		  ...
		  struct ice_acl_act_entry *acts, u8 acts_cnt, u16 *entry_idx)
{
	...
	*entry_idx = ice_acl_scen_assign_entry_idx(scen, prior);
	...
	entry_tcam = hw->acl_tbl->first_tcam +
		ICE_ACL_TBL_TCAM_IDX(scen->start) +
		(ICE_ACL_TBL_TCAM_ENTRY_IDX(scen->start) + *entry_idx) /
		ICE_AQC_ACL_TCAM_DEPTH * num_cscd;

	idx = ICE_ACL_TBL_TCAM_ENTRY_IDX(scen->start + *entry_idx);

	memset(&buf, 0, sizeof(buf));
	for (i = num_cscd - 1; i < num_cscd; i--) {
		...
		memcpy(&buf.entry_key.val,
		       &keys[i * sizeof(buf.entry_key.val)],
		       sizeof(buf.entry_key.val));
		memcpy(&buf.entry_key_invert.val,
		       &inverts[i * sizeof(buf.entry_key_invert.val)],
...
ice_acl_prog_act(struct ice_hw *hw, struct ice_acl_scen *scen,
		 ...
		 u16 entry_idx)
{
	u16 entry_tcam, num_cscd, i, idx, actx_idx = 0;
	struct ice_aqc_actpair act_buf;
	enum ice_status status = 0;

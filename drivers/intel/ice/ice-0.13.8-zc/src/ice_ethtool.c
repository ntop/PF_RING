	       ...
	       u8 *bytes)
{
	...
	if (status == ICE_ERR_AQ_ERROR &&
		...
		dev_err(dev, "ice_aq_read_nvm failed: %s %s\n",
			ice_stat_str(status),
			ice_aq_str(hw->adminq.sq_last_status));
		ret = -EIO;
		ice_release_nvm(hw);
...
static int ice_set_fec_cfg(struct net_device *netdev, enum ice_fec_mode req_fec)
{
	...
	if ((config.caps & ICE_AQ_PHY_ENA_AUTO_FEC) !=
		...
		status = ice_aq_set_phy_cfg(pi->hw, pi, &config, NULL);
		if (status) {
			err = -EAGAIN;
			goto done;
		}
...
#if defined(NETIF_F_HW_TC) || !defined(ETHTOOL_GFECPARAM)
ethtool_exit:
#endif /* NETIF_F_HW_TC || !ETHTOOL_GFECPARAM */
...
static int ice_set_channels(struct net_device *dev, struct ethtool_channels *ch)
{
	...
	if (ch->other_count != (test_bit(ICE_FLAG_FD_ENA, pf->flags)) ? 1 : 0)
		return -EINVAL;
...
static int ice_set_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	...
	if (vsi->type != ICE_VSI_PF || !ice_is_wol_ena(pf))
		return -EOPNOTSUPP;
	...
	/* Set WoL only if there is new value */

...
{
	u8 num_tcs = 0, i, tc = 0, up_mapped_tc, up_in_tc = 0;
...
int ice_pf_dcb_cfg(struct ice_pf *pf, struct ice_dcbx_cfg *new_cfg, bool locked)
{
	...
	old_cfg = (struct ice_dcbx_cfg *)kmemdup(curr_cfg, sizeof(*old_cfg),
						 GFP_KERNEL);
	if (!old_cfg)
		return -ENOMEM;
...
static int ice_dcb_init_cfg(struct ice_pf *pf, bool locked)
{
	...
	pi = pf->hw.port_info;
	newcfg = (struct ice_dcbx_cfg *)kmemdup(&pi->local_dcbx_cfg,
						sizeof(*newcfg), GFP_KERNEL);
	if (!newcfg)
		return -ENOMEM;

 ...
 * @rx_error: error bits extracted from desc qword
 * @rx_status: status bits extracted from desc qword
 * @inner_prot: inner protocol of decoded ptype
 * @is_ipv4: if ptype is ipv4 or not
 */
static void
ice_rx_extra_counters(struct ice_vsi *vsi, u32 rx_error, u32 rx_status,
		      u32 inner_prot, bool is_ipv4)
{
	if (is_ipv4) {
		vsi->back->rx_ip4_cso++;
		if (rx_error & (BIT(ICE_RX_DESC_ERROR_IPE_S) |
				BIT(ICE_RX_DESC_ERROR_EIPE_S)))
	...
	case ICE_RX_PTYPE_INNER_PROT_TCP:
		vsi->back->rx_tcp_cso++;
		if (rx_error & BIT(ICE_RX_DESC_ERROR_L4E_S))
...
#ifdef ICE_ADD_PROBES
	ice_rx_extra_counters(ring->vsi, rx_error, rx_status,
			      decoded.inner_prot, ipv4);
#endif

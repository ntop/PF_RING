 ...
 * @vsi: VSI to check
 */
bool ice_is_arfs_active(struct ice_vsi *vsi)
{
 ...
 * @skb: skb used to help build the aRFS entry
 * @flow_id: passed down from the stack and saved for flow expiration
 * @skb_proto: caller must pass either ETH_P_IP or ETH_P_IPV6
 * @rxq_idx: Rx queue to steer this flow to
 * @ip_proto: caller must pass either IPPROTO_TCP or IPPROTO_UDP
 *
 * returns an aRFS entry on success and NULL on failure
 */
static struct ice_arfs_entry *
ice_arfs_build_entry(struct ice_vsi *vsi, const struct sk_buff *skb,
		     u32 flow_id, u16 skb_proto, u16 rxq_idx, u8 ip_proto)
{
	...
	/* Have to swap src/dst for programming HW filters */
	if (skb_proto == ETH_P_IP) {
		fltr_info->ip.v4.proto = ip_proto;
		fltr_info->flow_type = (ip_proto == IPPROTO_TCP) ?
			ICE_FLTR_PTYPE_NONF_IPV4_TCP :
			ICE_FLTR_PTYPE_NONF_IPV4_UDP;
		fltr_info->ip.v4.src_ip = ip_hdr(skb)->daddr;
		fltr_info->ip.v4.dst_ip = ip_hdr(skb)->saddr;
		fltr_info->ip.v4.src_port = dst_port;
		...
		memcpy(&fltr_info->ip.v6.src_ip, &ipv6_hdr(skb)->daddr,
		       sizeof(struct in6_addr));
		memcpy(&fltr_info->ip.v6.dst_ip, &ipv6_hdr(skb)->saddr,
		       sizeof(struct in6_addr));
		fltr_info->ip.v6.src_port = dst_port;
		fltr_info->ip.v6.dst_port = src_port;
	}
 ...
 * @l3_proto: ETH_P_IP or ETH_P_IPV6
 ...
 */
static bool
ice_arfs_is_perfect_flow_set(struct ice_hw *hw, u16 l3_proto, u8 l4_proto)
{
	...
	if (l3_proto == ETH_P_IP && l4_proto == IPPROTO_UDP)
		return test_bit(ICE_FLTR_PTYPE_NONF_IPV4_UDP, perfect_fltr);
	else if (l3_proto == ETH_P_IP && l4_proto == IPPROTO_TCP)
		return test_bit(ICE_FLTR_PTYPE_NONF_IPV4_TCP, perfect_fltr);
	else if (l3_proto == ETH_P_IPV6 && l4_proto == IPPROTO_UDP)
		return test_bit(ICE_FLTR_PTYPE_NONF_IPV6_UDP, perfect_fltr);
	else if (l3_proto == ETH_P_IPV6 && l4_proto == IPPROTO_TCP)
		  ...
		  u16 rxq_idx, u32 flow_id)
{
	...
	u16 skb_proto = ntohs(skb->protocol);
	struct ice_arfs_entry *arfs_entry;
	...
	if (skb_proto == ETH_P_IP && !ip_is_fragment(ip_hdr(skb)))
	...
	if (!ice_arfs_is_perfect_flow_set(&pf->hw, skb_proto, ip_proto))
		return -EOPNOTSUPP;
	...
	arfs_entry = ice_arfs_build_entry(vsi, skb, flow_id, skb_proto,
					  rxq_idx, ip_proto);
	if (!arfs_entry) {
		ret = -ENOMEM;
		goto out;

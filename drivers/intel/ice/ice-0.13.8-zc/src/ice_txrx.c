...
bool ice_alloc_rx_bufs(struct ice_ring *rx_ring, u16 cleaned_count)
{
	...
	if (!rx_ring->netdev || !cleaned_count) {
		if (rx_ring->vsi->type == ICE_VSI_CTRL && cleaned_count)
...
{
	u32 total_fd_allowed = ch_vsi->num_gfltr + ch_vsi->num_bfltr;
...
#ifdef ADQ_PERF
/**
 * ice_is_ctrl_pkt - check if packet is a TCP control packet or data packet
 * @skb: receive buffer
 * @rx_ring: ptr to Rx ring
 *
 * Returns true for all unsupported protocol/configuration. Supported protocol
 * is TCP/IP.  For TCP/IPv6, this function returns true (means it is ctrl pkt)
 * if packet contains nested header.
 * Logic to determine control packet:
 * - packets is control packet if it contains flags like SYN, SYN+ACK, FIN, RST
 * - ACK with payload length=0, is not considered as control packets
 */
static bool ice_is_ctrl_pkt(struct sk_buff *skb, struct ice_ring *rx_ring)
{
	union {
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		payload_len =  ntohs(hdr.ipv4->tot_len) - (hlen + th->doff * 4);
		...
		/* payload_len is part of ipv6 header.
		 * payload_len : i.e. rest of the packet following this
		 * fixed size IPv6 header. 'payload_len' in IPv6 header
		 * includes any IPv6 extension headers + subsequent headers
		 * (such as L4 header) + actual payload. To figure out actual
		 * payload_len, subtract IPv6 extension headers, and subtract
		 * any subsequent headers (such as L4 headers). For now, not
		 * supporting IPv6 extension headers, hence only subtract
		 * L4 header size from IPv6.payload_len to obtain
		 * actual payload_len.
		 */
	...
	if ((th->fin || th->rst || th->syn) || (!payload_len && th->ack)) {
		/* it is either FIN/RST/SYN/!payload_len & ACK = ctrl pkt */
		ret_val = true;
		if (!payload_len && th->ack) {
			/* if receiving pure ACK (ACK with payload_len = 0,
			 * then after that there are two possibilities
			 * - SYN or FIN or RST, could have piggy backed with
			 * pure ACK, in that case - treat it like control pkt
			 * - Otherwise treat pure ACK (means nothing has been
			 * piggy-backed with pure ACK) as data packet
			 */
			if (!(th->fin || th->rst || th->syn))
				ret_val = false;
		}
		/* if FIN/RST was piggy-backed on pure ACK, treat the
		 * packet like "control packet" (hence ret_val = true).
		 * This is needed to ensure functional correctness, so that
		 * vector can transition into interrupt mode correctly based
		 * on logic of control packet versus data packet when
		 * busy_poll_stop is called.
		 */
	}

#ifdef ADQ_PERF_COUNTERS
	if (ret_val) {
		rx_ring->ch_q_stats.rx.num_tcp_ctrl_pkts++;
		if (th->fin)
	...
	/* select queue which will be used later during transmit path to setup
	 * inline-FD filter. Tx queue selection is done via. xps_rxqs sysfs
	 * is specified. Symmetric queue selection depends on xpq_rxqs map
	 * which is user configurable setting via. sysfs
	 * Without this logic, Tx queue selection happends based on sender_cpu,
	 * XPS map, prio_to_tc map, and usage of JHash if needed
	 */
	if (!th->ack && th->syn) {
...
construct_skb:
		...
		if (ice_vector_ch_enabled(rx_ring->q_vector)) {
			if (!ice_is_ctrl_pkt(skb, rx_ring))
	...
	if (rx_ring->vsi->type == ICE_VSI_CTRL)
		failure = ice_alloc_rx_bufs(rx_ring, total_rx_pkts);
	else
...
#else /* !ADQ_PERF */
		/* vector is not channel enabled and NAPI is not in BUSY_POLL,
...
static void ice_chnl_inline_fd(struct ice_ring *tx_ring, struct sk_buff *skb)
{
	...
	if (protocol == htons(ETH_P_IP)) {
		...
		/* unsupported protocol, neither ipv4 nor ipv6 */
		return;
	}
	...
	if (th->ack || th->fin || th->rst)  {
		/* server side || connection_termination */
		q_index = tx_ring->q_index;
		if (q_index >= ch->base_q)
			q_index -= ch->base_q;
		else
...
ice_xmit_frame_ring(struct sk_buff *skb, struct ice_ring *tx_ring)
{
	...
	if (ice_ring_ch_enabled(tx_ring))
		ice_chnl_inline_fd(tx_ring, skb);
#endif /* ADQ_PERF */

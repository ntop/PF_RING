...
static void irdma_alloc_push_page(struct irdma_qp *iwqp)
{
	...
	/* Upstream push mode is only enabled for GEN_3 and
	 * skipping push reuse isn't necessary
	 */
	if (iwqp->iwdev->rf->sc_dev.hw_attrs.uk_attrs.hw_rev < IRDMA_GEN_3)
		goto skip_push_reuse;
...
skip_push_reuse:
	...
	cqp_info->in.u.manage_push_page.info.qs_handle =
		qp->vsi->qos[qp->user_pri].qs_handle[qp->qs_idx];
...
#if !defined(COPY_USER_PGADDR_VER_1) || defined(UPSTREAM_RELEASE)
/**

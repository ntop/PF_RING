...
static void irdma_set_qos_info(struct irdma_sc_vsi  *vsi, struct irdma_l2params *l2p)
{
	...
	for (i = 0; i < IRDMA_MAX_USER_PRIORITY; i++) {
		if (vsi->dev->hw_attrs.uk_attrs.hw_rev == IRDMA_GEN_1)
			vsi->qos[i].qs_handle[IRDMA_LAG_PRIMARY_IDX] =
...
void irdma_qp_add_qos(struct irdma_sc_qp *qp)
{
	...
	if (!qp->on_qoslist) {
		...
		qp->qs_handle = vsi->qos[qp->user_pri].qs_handle[qp->qs_idx];
		ibdev_dbg(to_ibdev(qp->dev),
				  ...
				  u64 scratch, bool post_sq)
{
	...
	qw2 = FIELD_PREP(IRDMA_CQPSQ_QHASH_QS_HANDLE,
			 vsi->qos[info->user_pri].qs_handle[IRDMA_LAG_PRIMARY_IDX]);
	if (info->vlan_valid)
...
int irdma_sc_qp_init(struct irdma_sc_qp *qp, struct irdma_qp_init_info *info)
{
	...
	qp->qs_handle =
		qp->vsi->qos[qp->user_pri].qs_handle[IRDMA_LAG_PRIMARY_IDX];
			   ...
			   bool post_sq)
{
	...
	hdr = qp->qp_uk.qp_id |
	      ...
	      FIELD_PREP(IRDMA_CQPSQ_FWQE_ERR_SQ_IDX_VALID, info->err_sq_idx_valid) |
...
/**
 * irdma_sc_cqp_advance_effctv_sq_tail - update CQP SQ tail
 * @cqp: CQP HW structure
 *
 * If there are no pending/deferred completions, simply move tail by 1
 * or catch up with 'true' SQ tail.
 * Otherwise, SQ tail cannot move beyond wqe_idx of the first (oldest) pending
 * completion on the list.
 * SQ tail is advanced on each _final_ completion of any CQP op request.
 *
 * This function must be called with cqp->ooo_list_lock held.
 */
static void irdma_sc_cqp_advance_effctv_sq_tail(struct irdma_sc_cqp *cqp)
{
	if (!list_empty(&cqp->ooo_pnd)) {
		struct irdma_ooo_cqp_op *ooo_op;

		ooo_op = (struct irdma_ooo_cqp_op *)
			 list_entry(cqp->ooo_pnd.next,
				    struct irdma_ooo_cqp_op, list_entry);

		/* set tail to the oldest pending CQP op wqe idx */
		IRDMA_RING_SET_TAIL(cqp->sq_ring, ooo_op->wqe_idx);

		return;
	}

	/* no pending completions - catch up with the true tail position */
	IRDMA_RING_SET_TAIL(cqp->sq_ring,
			    IRDMA_RING_CURRENT_TRUE_TAIL(cqp->sq_ring));
}
	...
	list_for_each_entry(ooo_op, &dev->cqp->ooo_pnd, list_entry) {
		if (ooo_op->deferred &&
			...
			*scratch = ooo_op->scratch;
			...
			irdma_sc_cqp_advance_effctv_sq_tail(dev->cqp);
	...
	while (i++ < count) {
		...
		if (newtail != tail) {
			...
			if (cqp->dev->hw_attrs.uk_attrs.hw_rev >= IRDMA_GEN_3) {
				IRDMA_RING_MOVE_TRUE_TAIL(cqp->sq_ring);
				irdma_sc_cqp_advance_effctv_sq_tail(cqp);
			} else {
...
/**
 * irdma_sc_update_cqp_tail_stats - advance CQP SQ tail
 * @cqp: CQP sc struct
 * @move_true_tail: whether to advance SQ tail
 * @move_effctv_tail: whether to advance effective SQ tail and update
 *                    completed cmds stat
 */
static inline void irdma_sc_update_cqp_tail_stats(struct irdma_sc_cqp *cqp,
						  bool move_true_tail,
						  bool move_effctv_tail)
{
	unsigned long flags;
			      ...
			      struct irdma_ccq_cqe_info *info)
{
	...
	irdma_sc_update_cqp_tail_stats(cqp, !ooo_final_cmpl,
				       !def_cmpl && !pend_cmpl);
	spin_unlock_irqrestore(&cqp->dev->cqp_lock, flags);
	if (ooo_final_cmpl || def_cmpl)
		return ret_code;
					...
					u32 qpwanted)
{
	...
	hmc_info->hmc_obj[IRDMA_HMC_IW_RRF].cnt =
		min(hmc_info->hmc_obj[IRDMA_HMC_IW_RRF].max_cnt, IRDMA_RRF_MULTIPLIER * qpwanted);
